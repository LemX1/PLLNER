    ...
    for example in examples:
        ...
        if labels is not None:
            if type(labels[0]) == list:
                ...
                label_ids = [0]  # 首token为[CLS],label为O,id为0
                for label in labels:
                    label_ids.append(ent2id[label])
                label_ids.append(0)  # [SEP]label为O,id为0

                # pad
                if len(label_ids) < max_seq_len:
                    pad_length = max_seq_len - len(label_ids)
                    label_ids = label_ids + [0] * pad_length  # [PAD]label为O,id为0
